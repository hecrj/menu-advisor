...
(deffunction imprimir-menu (?menu)
    ...
    (if (= 2 ?nvinos)
        then (printout t "       vino: " (send (nth 1 $?vinos) get-nombre)))
    (printout t "    " (send ?segundo get-nombre) " (" (send ?segundo get-precio) " €)" crlf)
    (if (= 2 ?nvinos)
        then (printout t "       vino: " (send (nth 2 $?vinos) get-nombre)))
    (printout t "    " (send ?postre get-nombre)  " (" (send ?postre get-precio) " €)" crlf)
    (if (= 1 ?nvinos)
        then (printout t "    Vino para el menú: " (send (nth 1 $?vinos) get-nombre)))
    (printout t  crlf)
