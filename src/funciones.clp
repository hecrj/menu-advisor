...
(deffunction imprimir-recomendacion (?rec)
    ...
    (format t "      Precio: %.2f €" (send ?plato get-precio))
    (printout t crlf)
...
(deffunction imprimir-menu (?menu)
    ...
    (bind ?nvinos (length $?vinos))
    
    (printout t "    " (send ?primero get-nombre) " (")
    (format t "%.2f" (send ?primero get-precio))
    (printout t " €)" crlf)
    (if (= 2 ?nvinos)
        then (bind ?vino (nth 1 $?vinos))
             (printout t "       vino: " (send ?vino get-nombre) " (")
             (format t "%.2f" (send ?vino get-precio))
             (printout t " €)" crlf))

    (printout t "    " (send ?segundo get-nombre) " (")
    (format t "%.2f" (send ?segundo get-precio))
    (printout t " €)" crlf)
    (if (= 2 ?nvinos)
        then (bind ?vino (nth 2 $?vinos))
             (printout t "       vino: " (send ?vino get-nombre) " (")
             (format t "%.2f" (send ?vino get-precio))
             (printout t " €)" crlf))

    (printout t "    " (send ?postre get-nombre) " (")
    (format t "%.2f" (send ?postre get-precio))
    (printout t " €)" crlf)
    (if (= 1 ?nvinos)
        then (bind ?vino (nth 1 $?vinos))
             (printout t "       vino: " (send ?vino get-nombre) " (")
    ...
    (printout t  crlf)
    (printout t "  Precio total: " (format nil "%.2f €" (send ?menu get-precio)) crlf)
