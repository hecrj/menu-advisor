...
(defrule seleccionar-menus
    ...
    (if (and (>= ?precio ?min) (<= ?precio ?bar))
        then (if (= 0 (length $?b))
                 ...
                 else (if (> ?puntuacion (send (instance-address * (nth 1 $?b)) get-puntuacion))
                          then (modify ?sel (barato $?ml)))))
    (if (and (> ?precio ?bar) (<= ?precio ?med))
        then (if (= 0 (length $?m))
                 then (modify ?sel (medio $?ml))
                 else (if (> ?puntuacion (send (instance-address * (nth 1 $?m)) get-puntuacion))
                          then (modify ?sel (medio $?ml)))))
    (if (and (> ?precio ?med) (<= ?precio ?car))
        then (if (= 0 (length $?c))
                 then (modify ?sel (caro $?ml))
                 else (if (> ?puntuacion (send (instance-address * (nth 1 $?c)) get-puntuacion))
                          then (modify ?sel (caro $?ml)))))
